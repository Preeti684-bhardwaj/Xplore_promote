...
const getQRSession = async (channel,userId) => {
    const channelData= await db.qrSessions.findOne({ where: { channel:channel } });
    ...
    try {
      ...
      if (!userSession) {
        return {
          success: false,
          status: 404,
          message: `Session with channel ${channel} doesn't exist for user ${userId}`,
        };
      }
      ...
      // Delete the session
      await db.qrSessions.destroy({ where: { channel, userId } });
      ...
      return {
        success: false,
        status: 500,
        message: "An error occurred during session deletion",
      };
    }
  };
